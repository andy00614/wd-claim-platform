...
const ExpenseAnalysisSchema = z.object({
  date: z
    .string()
    .optional()
    .describe("Date in MM/dd format from the expense document"),
  itemNo: z
    .string()
    .optional()
    .describe("Expense category code like A1, A2, B1, C1, C2, etc."),
  details: z
    .string()
    .optional()
    .describe("Expense description including vendor/restaurant name"),
  currency: z
    .string()
    .optional()
    .describe("Currency code like SGD, USD, EUR, etc."),
  amount: z.string().optional().describe("Expense amount as string"),
  forexRate: z
    .string()
    .optional()
    .describe("Exchange rate as string, calculated from exchange rates"),
  sgdAmount: z
    .string()
    .optional()
    .describe("SGD amount as string, calculated from amount and rate"),
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes("image")) {
        return NextResponse.json(
          {
            error:
              "Image format not supported or corrupted. Please try with a clear image.",
          },
          { status: 400 },
        );
      }
    ...
    return NextResponse.json(
      {
        error:
          "Failed to analyze image. Please try again or fill in the form manually.",
      },
      { status: 500 },
    );
  }
