      ...
      if (attachments && attachments.length > 0) {
        attachments.forEach((attachment) => {
          if (
            ...
            !combined.some((existing) => existing.id === attachment.id)
          ) {
      ...
      return (
        ...
        <img
          ...
          onError={(event) => {
            ...
            fallback.className =
              "p-4 bg-gray-200 text-center text-sm text-gray-500 rounded";
...
<body>
  <div class="report-container">
    ...
    link.href = url;
    link.download = `Expense_Claim_Report_${formatClaimId(claim.id)}_${employee.name}.html`;
        ...
        <div id="report-content" className="report-wrapper space-y-6">
            ...
            <div className="claim-header-grid gap-6 grid grid-cols-1 lg:grid-cols-[2fr_1fr]">
              <div>
                <div className="claim-header-table border-2 border-black rounded-lg overflow-hidden">
                  <div className="claim-header-title text-center font-bold text-xl border-b-2 border-black bg-slate-100 px-4 py-3">
                    CLAIM FORM
                  </div>
                  <div className="claim-header-row grid grid-cols-1 sm:grid-cols-[1fr_1.2fr] border-b border-black last:border-b-0">
                    <div className="claim-header-label px-4 py-2 font-semibold bg-emerald-50 text-sm border-black border-b sm:border-b-0 sm:border-r">
                      Employee Code
                    </div>
                    <div className="claim-header-value px-4 py-2 bg-emerald-100 text-sm font-medium">
                      {employee.employeeCode
                        ? `WD0${employee.employeeCode}`
                        : "—"}
                    </div>
                  </div>
                  <div className="claim-header-row grid grid-cols-1 sm:grid-cols-[1fr_1.2fr] border-b border-black last:border-b-0">
                  ...
                  <div className="claim-header-row grid grid-cols-1 sm:grid-cols-[1fr_1.2fr] border-b border-black last:border-b-0">
                    ...
                    <div className="claim-header-value amount px-4 py-2 bg-emerald-100 text-base font-semibold">
                      SGD {claimedAmountDisplay}
                    </div>
                  </div>
                  <div className="claim-header-row grid grid-cols-1 sm:grid-cols-[1fr_1.2fr]">
                    <div className="claim-header-label px-4 py-2 font-semibold bg-emerald-50 border-black border-b sm:border-b-0 sm:border-r text-sm">
                      Posting Date
                    </div>
                    <div className="claim-header-value px-4 py-2 bg-emerald-100 text-sm font-medium">
                      {postingDateDisplay}
                ...
                <p className="claim-header-footnote text-right text-xs text-slate-500 italic mt-2">
                  dd/mm/yyyy
                </p>
              </div>

              <div className="ref-panel border-2 border-black rounded-lg flex flex-col overflow-hidden h-full">
                <div className="ref-panel-title px-4 py-3 font-bold text-lg border-b-2 border-black">
                  Ref Number
                </div>
                <div className="ref-panel-value px-4 py-6 font-mono font-semibold text-lg">
                  {formatClaimId(claim.id)}
                </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                <h3 className="text-lg font-semibold text-slate-900">
                  Itemised Claim Breakdown
                </h3>
                <p className="text-sm text-slate-500">
                  All expense items are displayed below with their supporting
                  information and documents for easy review and printing.
                </p>
              </div>

              <div className="space-y-6">
                {items.length === 0 ? (
                  <div className="preview-placeholder border border-dashed border-blue-200 rounded-xl bg-blue-50/60 p-6 text-center text-sm text-slate-500">
                    No expense items available.
                  </div>
                ) : (
                  items.map((item, index) => {
                    const itemAttachments =
                      attachmentsByItemId.get(item.id) ?? [];
                    ...
                    const additionalNotes =
                      item.details && item.note && item.details !== item.note
                        ? item.details
                        : null;
                    ...
                    return (
                      <article
                        key={item.id}
                        className="item-card avoid-break rounded-xl border border-slate-200 bg-white shadow-sm p-5 sm:p-6 space-y-5 print:shadow-none"
                      >
                        <div className="item-card-header flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          ...
                          <div className="sm:text-right">
                            ...
                            <p className="text-xl font-semibold text-emerald-600 font-mono">
                              SGD{" "}
                              {Number.parseFloat(item.sgdAmount || "0").toFixed(
                                2,
                              )}
                            </p>
                          </div>
                        ...
                        <div className="item-metadata-grid grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                          <div>
                            <p className="item-metadata-label text-xs font-semibold uppercase tracking-wide text-slate-500">
                              Invoice Number
                            </p>
                            <p className="item-metadata-value text-sm font-mono text-slate-800">
                              {item.evidenceNo || "—"}
                            </p>
                          </div>
                          <div>
                          ...
                          {additionalNotes && (
                            <div className="item-metadata-note xl:col-span-3">
                              ...
                              <p className="item-metadata-value text-sm text-slate-700">
                                {additionalNotes}
                              </p>
                            </div>
                          )}
            ...
            <section className="section-card report-section bg-white border border-slate-200 shadow-sm rounded-xl p-6 sm:p-8">
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-sm text-slate-500">
                    These documents are linked to the claim as a whole and are
                ...
                <div className="item-attachment-grid grid gap-4 md:grid-cols-2">
                  {claimLevelAttachments.map((attachment) => (
                    <div
                      key={attachment.id}
                      className="attachment-card rounded-lg border border-slate-200 overflow-hidden bg-white"
                    >
          ...
          <section className="section-card report-section bg-white border border-slate-200 shadow-sm rounded-xl p-6 sm:p-8">
            <div className="flex flex-col gap-2">
              <h3 className="text-lg font-semibold text-slate-900">
                Editable Accounting Table
              </h3>
              <p className="text-sm text-slate-500">
                Adjust the export-ready table below. Required Xero columns are
            ...
            <div className="editable-table-wrapper overflow-x-auto mt-4">
              {editableRows.length === 0 ? (
                <div className="preview-placeholder border border-dashed border-blue-200 rounded-xl bg-blue-50/60 p-6 text-center text-sm text-slate-500">
                  No expense items available to populate the table.
                </div>
              ) : (
