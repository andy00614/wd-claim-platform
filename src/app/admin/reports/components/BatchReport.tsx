...
<body>
  ...
  const totalAmount = claims.reduce(
    (sum, claimData) => sum + parseFloat(claimData.claim.totalAmount),
    0,
  );
  const totalItems = claims.reduce(
    (sum, claimData) => sum + claimData.items.length,
    0,
  );
      ...
      <div id="batch-report-content" className="max-w-6xl mx-auto bg-white p-8">
        ...
        <div className="text-center border-2 border-black p-6 mb-8">
          ...
          <p className="text-sm text-gray-600 mt-2">
            Generated on: {new Date().toLocaleDateString("en-SG")} at{" "}
            {new Date().toLocaleTimeString("en-SG")}
          </p>
          <p className="text-sm text-gray-600">
            Total Claims: {claims.length} | Total Amount: SGD{" "}
              ...
              {claims.map((claimData) => (
                <tr key={claimData.claim.id}>
                  <td className="border border-black p-3">
                    CL-2024-{claimData.claim.id.toString().padStart(4, "0")}
                  </td>
                  <td className="border border-black p-3">
                    ...
                    <span className="text-sm text-gray-600">
                      EMP
                      {claimData.employee.employeeCode
                        .toString()
                        .padStart(3, "0")}
                    </span>
                  </td>
                  ...
                  <td className="border border-black p-3">
                    {claimData.claim.createdAt
                      ? new Date(claimData.claim.createdAt).toLocaleDateString(
                          "en-SG",
                        )
                      : "N/A"}
                  </td>
                  <td className="border border-black p-3 text-center">
                    {claimData.items.length}
                  </td>
                  <td className="border border-black p-3 text-right font-medium">
                    {parseFloat(claimData.claim.totalAmount).toFixed(2)}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {/* 详细申请报告 */}
        <div>
          <h3 className="text-lg font-bold mb-6 border-b border-black pb-2">
            Detailed Claims
          </h3>
          ...
          {claims.map((claimData, index) => (
            <div
              ...
              className={`mb-12 ${index > 0 ? "page-break" : ""}`}
            >
              <div className="border-2 border-gray-400 p-6">
                ...
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-300">
                  <div>
                    <h4 className="text-xl font-bold">
                      Claim CL-2024-
                      {claimData.claim.id.toString().padStart(4, "0")}
                    </h4>
                    <p className="text-gray-600">
                      {claimData.employee.name} (EMP
                      {claimData.employee.employeeCode
                        .toString()
                        .padStart(3, "0")}
                      )
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold">
                      SGD {parseFloat(claimData.claim.totalAmount).toFixed(2)}
                    </div>
                    <div className="text-sm text-gray-600">
                      {claimData.items.length} items
                    </div>
                  </div>
                </div>
                ...
                <table className="w-full border-collapse border border-gray-300 mb-6">
                  <thead>
                    <tr className="bg-gray-50">
                      <th className="border border-gray-300 p-2 text-left text-sm font-bold">
                        Date
                      </th>
                      <th className="border border-gray-300 p-2 text-left text-sm font-bold">
                        Type
                ...
                {/* 支持文档（仅显示图片，节省空间） */}
                {(claimData.attachments?.some((att) =>
                  isImageFile(att.fileType, att.fileName),
                ) ||
                  claimData.items.some((item) =>
        ...
        <div className="mt-16 page-break">
          ...
          <div className="grid grid-cols-1 gap-8">
            <div>
              <div className="mb-4">
                <span className="font-bold">Total Claims Reviewed:</span>{" "}
                {claims.length}
              </div>
              <div className="mb-4">
                <span className="font-bold">Total Amount Approved:</span> SGD{" "}
                {totalAmount.toFixed(2)}
              </div>
              <div className="mb-4">
        ...
        <div className="mt-12 pt-4 border-t border-gray-300 text-center text-xs text-gray-500">
          ...
          <p>
            This report contains {claims.length} approved expense claims
            totaling SGD {totalAmount.toFixed(2)}.
          </p>
          <p>
            Generated for internal audit and accounting review purposes only.
          </p>
        </div>
      </div>
    </>
