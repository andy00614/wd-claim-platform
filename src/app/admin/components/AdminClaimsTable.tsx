...
export default function AdminClaimsTable({ claims }: AdminClaimsTableProps) {
  ...
  const formatSingaporeTime = (value: Claim["createdAt"]) => {
    if (!value) return "N/A";

    return dayjs.utc(value).tz("Asia/Singapore").format("YYYY-MM-DD HH:mm");
  };
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {formatSingaporeTime(claim.createdAt)}
                  </div>
                </div>
          ...
          <Table className="min-w-[960px]">
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="sticky right-0 z-10 w-12 bg-background border-l">
                  Actions
                </TableHead>
              </TableRow>
            </TableHeader>
              ...
              {claims.length > 0 ? (
                claims.map((claim) => (
                  <TableRow key={claim.id} className="group">
                    ...
                    <TableCell className="sticky right-0 bg-background border-l group-hover:bg-muted/50">
                      {editingClaim === claim.id ? (
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48">
                            ...
                            <DropdownMenuItem asChild>
                              <Link
                                href={`/admin/reports/v2/${claim.id}`}
                                className="flex items-center gap-3 cursor-pointer py-2.5 px-3"
                              >
                                <BarChart3 className="h-4 w-4 text-green-600" />
                                <span className="text-sm font-medium text-gray-700">
