...
export default function EditClaimForm({
  ...
  const [expenseItems, setExpenseItems] =
    useState<ExpenseItem[]>(convertedItems);
  const [attachedFiles, setAttachedFiles] = useState<File[]>([]);

  // 绑定updateClaim到当前claimId
  const updateClaimWithId = updateClaim.bind(null, claimId);
  const [state, formAction] = useActionState(updateClaimWithId, {
    success: false,
    error: "",
  });
  ...
  useEffect(() => {
    if (state.success && state.data?.claimId) {
      alert(
        `申请更新成功！申请ID: CL-2024-${state.data.claimId.toString().padStart(4, "0")}`,
      );
      // 跳转回详情页
  ...
  return (
    <form action={formAction}>
      ...
      <div className="text-center mt-6">
        ...
        <SubmitButton
          hasItems={expenseItems.length > 0}
