  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const claimItemsData = expenseItems.map((item, index) => {
        ...
        const [rawCurrency] = currencyCode.split(" ");
        const normalizedCurrency = rawCurrency?.trim() ?? "";
        const currencyId =
          currencyMap[currencyCode] ?? currencyMap[normalizedCurrency];
        if (!currencyId) {
          throw new Error(`无效的货币代码 (index ${index}): ${currencyCode}`);
        }
