...
import { claims, claimItems, attachment, employees } from '../src/lib/db/schema'
...
async function checkClaimsData() {
  try {
    ...
    // 按状态分组统计
    const statusStats = allClaims.reduce((acc, claim) => {
    ...
    if (recentClaims.length > 0) {
      ...
      recentClaims.forEach(claim => {
        ...
        console.log(`${claim.id.toString().padEnd(4)} | ${claim.status.padEnd(10)} | SGD ${parseFloat(claim.totalAmount.toString()).toFixed(2).padEnd(6)} | ${(claim.employeeName || 'Unknown').padEnd(8)} | ${date}`)
      })
    } else {
      console.log('  暂无申请记录')
    }
